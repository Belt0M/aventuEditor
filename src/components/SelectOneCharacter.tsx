...
const SelectOneCharacter: React.FC<Props> = ({
	options,
	maxHeight = 'max-h-24',
	name,
	setCharacter,
	...
	const [selectedValue, setSelectedValue] = useState<string>(character.voice!)
	const [searchTerm, setSearchTerm] = useState<string>('')

	useEffect(() => {
		setSelectedValue(character.voice!)
	}, [character.voice])
	...
	return (
		<div className='flex flex-col gap-2'>
			...
			<div className='relative mb-2 text-left select-input'>
				<div>
					<button
						type='button'
						value={selectedValue}
						className={clsx(
							...
							'flex justify-between items-center w-full px-4 py-[0.435rem] text-sm font-medium  bg-white border-2 border-yellow border-opacity-40 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-0 focus:border-blue-300'
						)}
...
export default SelectOneCharacter
